...
> **This fork adds a third option, Composio, and recommends it.** No Meta app, no App
> Review, no paid provider, and it runs on Vercel Hobby or Docker. Follow the
> [README](../README.md) and [composio.md](composio.md) instead of this guide. The rest of
> this page is the upstream guide for the Meta and Zernio connections.

| Connection | What you configure | Costs |
| --- | --- | --- |
| **[Composio](composio.md), this fork's default** | A Composio API key and one connected Instagram account, saved with `scripts/connect-composio.ts`. | Composio free plan, plus your hosting. Comment triggers only. |
