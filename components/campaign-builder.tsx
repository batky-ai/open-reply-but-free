...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  const commentOnly = accounts.find((a) => a.id === selectedAccountId)?.provider === "COMPOSIO";
  const username =
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (!commentOnly && openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
    ...
    const payload = {
      ...
      dmTriggerEnabled: !commentOnly && dmTriggerEnabled,
      dmMessage,
      openingDmEnabled: !commentOnly && openingDmEnabled,
      ...
      requireFollow: !commentOnly && requireFollow,
      ...
      followPromptButtonLabel: requireFollow
        ...
        : "",
      followUpEnabled: !commentOnly && followUpEnabled,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            {saving ? "Saving…" : mode === "new" ? "Save paused" : "Save changes"}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-8 min-w-0">
        ...
        <Section title="1. Choose a post or reel">
          <Radio
        ...
        <Section title="2. Enter keywords">
          <Radio
          ...
          <details className="space-y-3"><summary className="cursor-pointer text-sm text-muted">{commentOnly ? "Advanced: public replies" : "Advanced: story, DM, and public replies"}</summary>
          {!commentOnly && <>
        ...
        {!commentOnly && (<details className="space-y-3"><summary className="cursor-pointer text-sm text-muted">Advanced: opening message and follow requirement</summary>
            ...
            {openingDmEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Thanks for your interest. Tap below for the link."
                  rows={3}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Follow us, then tap below to get the link."
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
        ...
        <Section title="3. Write your reply and add a link">
          <div className="rounded-lg border border-border p-3 space-y-2">
            ...
            <p className="text-xs text-muted">Paste a destination URL or a short link from the Links tool.</p>
            <p className="text-xs text-muted">
              {"{link}"} inserts the tracked link; {"{username}"} personalizes.
            </p>
          </div>
          {!commentOnly && (<details><summary className="cursor-pointer text-sm text-muted">Advanced: follow-up message</summary>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Thanks for following along."
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
