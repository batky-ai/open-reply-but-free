...
#             runs off Vercel (see deploy/compose.yml). It needs scripts/
#             in the image and wget on PATH; node:22-slim ships neither.
#
...
# `command: ["npm", "run", "worker"]` (see deploy/compose.yml).
CMD ["npm", "run", "start"]
