  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <section className="panel rounded p-4 sm:p-6">
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted">
                    {account.provider === "COMPOSIO" ? "Connected via Composio · Checks comments every 5 minutes" : account.provider === "ZERNIO" ? "Connected via Zernio" : <>Token expires{" "}
                    {account.tokenExpiresAt
                      ? new Date(account.tokenExpiresAt).toLocaleDateString()
                      : "not available"}</>}{" "}
                    {account.provider !== "COMPOSIO" && <>· {account.webhookSubscribed ? "Webhook ready" : "Webhook pending"}</>}
                  </p>
                </div>
                <button
        ...
        {!accounts.some((account) => account.provider === "COMPOSIO") && <div className="mt-6 pt-4 border-t border-border flex gap-3">
          <a
