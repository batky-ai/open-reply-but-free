import { unsupportedComposioFeatures } from "@/lib/composio/campaigns";
...
export async function POST(request: NextRequest) {
  ...
  if (unsupportedComposioFeatures(instagramAccount.provider, parsed.data)) {
    return NextResponse.json({ success: false, error: "This connection supports comment replies. DM triggers, opening messages, follow gates and follow-ups require webhooks." }, { status: 400 });
  }
...
export async function PATCH(request: NextRequest) {
  ...
  const existing = await prisma.automation.findFirst({
    ...
    include: { instagramAccount: { select: { provider: true } } },
  });
  ...
  if (unsupportedComposioFeatures(existing.instagramAccount.provider, { ...existing, ...parsed.data })) {
    return NextResponse.json({ success: false, error: "This connection supports comment replies. Turn off features that require webhooks." }, { status: 400 });
  }
  ...
  const updated = await prisma.automation.update({
    ...
    data: { ...automationData, ...(automationData.isActive === true && !existing.isActive ? { activatedAt: new Date() } : {}) },
  });
