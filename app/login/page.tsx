...
import { isPublicDemoHost, isEmailAllowedToSignIn } from "@/lib/env";
...
export default async function LoginPage({
  ...
  const templateCallbackUrl = selectedTemplate
    ...
    : null;
  const callbackUrl = params.callbackUrl ?? templateCallbackUrl ?? "/campaigns";

  async function sendMagicLink(formData: FormData) {
    "use server";
    const email = String(formData.get("email") ?? "").trim().toLowerCase();
    if (!isEmailAllowedToSignIn(email)) redirect("/login?error=access");
    try {
      await signIn(EMAIL_PROVIDER_ID, { email, redirectTo: callbackUrl });
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted text-sm leading-relaxed mt-2">
            {selectedTemplate
              ...
              : "Manage keywords, links, and Instagram replies."}
          </p>
        </div>
        ...
        <div className="panel rounded p-8 shadow-black/40">
          {params.error && <p role="alert" className="mb-5 rounded border border-error/20 bg-error/10 p-3 text-sm text-error">{params.error === "access" ? "This email does not have access. Use an approved email address or contact Evan." : "We could not send your sign-in link. Please try again or contact Evan."}</p>}
            ...
            <form action={sendMagicLink} className="space-y-5">
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  autoComplete="email"
                  placeholder="Your approved email address"
