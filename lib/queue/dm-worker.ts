...
import { ComposioDeliveryUnconfirmedError } from "@/lib/composio/transport";
...
function isTemplateRejection(error: unknown): boolean {
  if (
    ...
    error instanceof ComposioDeliveryUnconfirmedError
  ) {
    return false;
  }
...
async function processComment(job: Job<ProcessCommentJob>): Promise<void> {
  ...
  for (const automation of automations) {
    if (automation.instagramAccount.provider === "COMPOSIO" &&
      (!commentAfterActivation(job.data.commentTimestamp, automation.activatedAt, automation.createdAt) ||
       unsupportedComposioFeatures("COMPOSIO", automation))) continue;
      ...
      try {
        ...
        await prisma.dmLog
          .update({
            ...
            data: { publicReplyError: formatError(error), publicReplyDeliveryUnconfirmed: (error instanceof ZernioDeliveryUnconfirmedError || error instanceof ComposioDeliveryUnconfirmedError) },
          })
          .catch(() => {});
      }
    }
    ...
    try {
      ...
      await prisma.dmLog.update({
        ...
        data: {
          ...
          dmDeliveryUnconfirmed: (error instanceof ZernioDeliveryUnconfirmedError || error instanceof ComposioDeliveryUnconfirmedError),
        },
      });
      throw error;
    }
...
async function sendPostbackOnce({
  ...
  try {
    ...
    throw (error instanceof ZernioDeliveryUnconfirmedError || error instanceof ComposioDeliveryUnconfirmedError)
      ? error
...
async function processPostback(job: Job<ProcessPostbackJob>): Promise<void> {
  ...
  try {
    ...
    if (fallback && !((error instanceof ZernioDeliveryUnconfirmedError || error instanceof ComposioDeliveryUnconfirmedError))) {
      console.log(
...
async function processJob(job: Job<DmQueueJob>): Promise<void> {
  try {
    ...
    if ((error instanceof ZernioDeliveryUnconfirmedError || error instanceof ComposioDeliveryUnconfirmedError))
