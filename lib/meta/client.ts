import { graphFetch, type GraphCredentials, bearerHeaders, setAccessToken } from "@/lib/composio/transport";
...
export async function sendPrivateReply(
  accessToken: GraphCredentials,
  instagramAccountId: string,
  commentId: string,
  message: string
): Promise<{ recipient_id: string; message_id: string }> {
  const response = await graphFetch(accessToken,
...
export async function getMediaComments(
  ...
  setAccessToken(url, accessToken);
...
/** Nested replies may contain only IDs; the reply itself exposes its author. */
export async function getCommentAuthor(
  accessToken: GraphCredentials,
  commentId: string
): Promise<{ id: string; username?: string }> {
  const url = new URL(`${instagramGraphBase()}/${encodeURIComponent(commentId)}`);
  url.searchParams.set("fields", "id,from,username");
  setAccessToken(url, accessToken);
  const data = await handleResponse<{ from?: { id?: string; username?: string } }>(
    await graphFetch(accessToken, url.toString(), { signal: AbortSignal.timeout(10_000) })
  );
  if (!data.from?.id) throw new Error("Instagram reply author is unavailable; defer this comment until the next check.");
  return { id: data.from.id, username: data.from.username };
}
...
export async function getRecentMediaComments(
  ...
  while (nextUrl !== null && results.length < max) {
    const response: Response = await graphFetch(accessToken, nextUrl);
...
export async function getConversations(
  ...
  async function readPage(limit: number, requestedFields: string): Promise<Page> {
    ...
    return handleResponse<Page>(
      await graphFetch(accessToken, url.toString(), {
        headers: { ...bearerHeaders(accessToken) },
...
export async function getUserInfo(accessToken: GraphCredentials): Promise<InstagramUser> {
