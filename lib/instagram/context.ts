import { type ComposioCredentials, parseComposioCredentials } from "@/lib/composio/transport";
import { decryptToken } from "@/lib/meta/oauth";
import { prisma } from "@/lib/db/client";

export type InstagramContext =
  | { provider: "META"; accessToken: string }
  | { provider: "COMPOSIO"; accessToken: ComposioCredentials }
  | {
      provider: "ZERNIO";
      apiKey: string;
      accountId: string;
...
export async function createInstagramContext(
  ...
  if (account.provider === "COMPOSIO")
    return { provider: "COMPOSIO", accessToken: { ...parseComposioCredentials(decryptToken(account.accessToken)), operationId } };
