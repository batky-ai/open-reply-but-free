...
export async function getCommentAuthor({ context, commentId }: {
  context: InstagramContext;
  commentId: string;
}) {
  if (context.provider === "ZERNIO")
    throw new Error("Instagram reply author is unavailable; defer this comment until the next check.");
  return meta.getCommentAuthor(context.accessToken, commentId);
}
...
export async function getAllUserMedia({
  ...
  return context.provider !== "ZERNIO"
...
export async function getUserInfo({
  ...
  if (context.provider !== "ZERNIO") return meta.getUserInfo(context.accessToken);
  const result = await zernioRequest<{
    accounts: {
      _id: string;
