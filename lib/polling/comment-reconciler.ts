...
/** Never interpret a missing nested author as evidence that nobody replied. */
export async function hasOwnerReply(
  context: InstagramContext,
  comment: InstagramComment,
  ownerId: string
): Promise<boolean> {
  const replies = comment.replies?.data ?? [];
  if (replies.some((reply) => reply.from?.id === ownerId)) return true;
  for (const reply of replies) {
    if (reply.from?.id) continue;
    const author = await getCommentAuthor({ context, commentId: reply.id });
    if (!author?.id) throw new Error("Instagram reply author is unavailable; defer this comment until the next check.");
    if (author.id === ownerId) return true;
  }
  return false;
}
...
export async function reconcileComments(): Promise<void> {
  ...
  for (const automation of automations) {
    const stat = await sweepCampaign({
      ...
      sinceMs: automation.instagramAccount.provider === "COMPOSIO"
        ? Math.max(sinceMs, (automation.activatedAt ?? automation.createdAt).getTime()) : sinceMs,
...
async function sweepCampaign({
  ...
  for (const mediaId of mediaIds) {
    ...
    let enqueuedForMedia = 0;
    for (const c of fresh) {
      if (enqueuedForMedia >= MAX_NEW_PER_SWEEP) break;
      try {
        if (await hasOwnerReply(accessToken, c, account.instagramId)) {
          stat.alreadyReplied += 1;
          continue;
        }
      } catch (error) {
        // A later sweep retries this read. No send is attempted while authorship
        // is unknown, and a failed lookup does not block unrelated comments.
        stat.errors.push(`Reply check ${c.id}: ${errMessage(error)}`);
        continue;
      }
